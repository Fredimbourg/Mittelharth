...
"""
fetch_and_build.py
Station Colmar-Mittelharth
Récupère les données Ecowitt API et génère les pages HTML du site.
"""
...
# ── Clés API (injectées par GitHub Actions via les secrets) ──────────────────
...
def fetch_realtime():
    ...
    live = {
        ...
        "temp":        fv(d, "outdoor", "temperature", "value"),
        "temp_feels":  fv(d, "outdoor", "feels_like", "value"),
        "hum":         fv(d, "outdoor", "humidity", "value"),
        "dew":         fv(d, "outdoor", "dew_point", "value"),
        "pressure":    fv(d, "pressure", "relative", "value"),
        "wind_speed":  fv(d, "wind", "wind_speed", "value"),
        "wind_gust":   fv(d, "wind", "wind_gust", "value"),
        "wind_dir":    fv(d, "wind", "wind_direction", "value"),
        "rain_rate":   fv(d, "rainfall", "rain_rate", "value"),
        "rain_daily":  fv(d, "rainfall", "daily", "value"),
        "rain_monthly":fv(d, "rainfall", "monthly", "value"),
        "rain_yearly": fv(d, "rainfall", "yearly", "value"),
        "solar":       fv(d, "solar_and_uvi", "solar", "value"),
        "uvi":         fv(d, "solar_and_uvi", "uvi", "value"),
        "temp_in":     fv(d, "indoor", "temperature", "value"),
        "hum_in":      fv(d, "indoor", "humidity", "value"),
    }
...
# ── 2. Historique (30 derniers jours + agrégats mensuels) ────────────────────
def fetch_history():
    ...
    if data.get("code") != 0:
        raise RuntimeError(f"API history error: {data}")

    # Debug : afficher la structure brute pour comprendre le format
    raw = data.get("data", {})
    print(f"  → Structure API: {list(raw.keys()) if isinstance(raw, dict) else type(raw)}")

    # L'API Ecowitt peut renvoyer soit un dict {ts: {value}} soit une liste [{time, value}]
    # On normalise tout en dict {ts_str: {value_key: val}}
    def normalize(obj):
        """Convertit list ou dict en dict {timestamp_str: données}"""
        ...
        if isinstance(obj, list):
            ...
            for item in obj:
                if isinstance(item, dict):
                    # Format liste : [{time: ts, value: v}] ou [{time: ts, avg: v, max: v, min: v}]
                    ts = str(item.get("time") or item.get("ts") or item.get("timestamp", ""))
                    if ts:
                        result[ts] = item
            return result
        return {}
    ...
    # Extraire les séries selon la structure
    def get_series(d, *path):
        """Navigue dans le dict et normalise le résultat final."""
        obj = d
        for key in path:
    ...
    temps  = get_series(d, "outdoor", "temperature")
    hums   = get_series(d, "outdoor", "humidity")
    rains  = get_series(d, "rainfall", "daily")
    solars = get_series(d, "solar_and_uvi", "solar")
    presss = get_series(d, "pressure", "relative")
    winds  = get_series(d, "wind", "wind_speed")

    print(f"  → Entrées temp: {len(temps)}, pluie: {len(rains)}, solaire: {len(solars)}")

    # Construire liste journalière — regrouper par date
    from collections import defaultdict as dd2
    by_date = dd2(lambda: {"avg":[],"hi":[],"lo":[],"hum":[],"rain":[],"solar":[],"pres":[],"wind":[]})

    def extract_val(item, *keys):
        """Extrait une valeur numérique depuis un item (dict ou valeur directe)."""
    ...
    for ts, item in temps.items():
        try:
            dt = datetime.datetime.fromtimestamp(int(float(ts)))
            date_str = dt.strftime("%Y-%m-%d")
            v = extract_val(item, "avg", "value")
            if v is not None: by_date[date_str]["avg"].append(v)
            v = extract_val(item, "max")
            if v is not None: by_date[date_str]["hi"].append(v)
            v = extract_val(item, "min")
            if v is not None: by_date[date_str]["lo"].append(v)
        except: continue

    for ts, item in hums.items():
        try:
            dt = datetime.datetime.fromtimestamp(int(float(ts)))
            date_str = dt.strftime("%Y-%m-%d")
            v = extract_val(item, "avg", "value")
            if v is not None: by_date[date_str]["hum"].append(v)
        except: continue

    for ts, item in rains.items():
        try:
            dt = datetime.datetime.fromtimestamp(int(float(ts)))
            date_str = dt.strftime("%Y-%m-%d")
            v = extract_val(item, "value", "sum", "avg")
            if v is not None: by_date[date_str]["rain"].append(v)
        except: continue

    for ts, item in solars.items():
        try:
            dt = datetime.datetime.fromtimestamp(int(float(ts)))
            date_str = dt.strftime("%Y-%m-%d")
            v = extract_val(item, "avg", "value")
            if v is not None: by_date[date_str]["solar"].append(v)
        except: continue

    for ts, item in presss.items():
        try:
            dt = datetime.datetime.fromtimestamp(int(float(ts)))
            date_str = dt.strftime("%Y-%m-%d")
            v = extract_val(item, "avg", "value")
            if v is not None: by_date[date_str]["pres"].append(v)
        except: continue

    for ts, item in winds.items():
        try:
            dt = datetime.datetime.fromtimestamp(int(float(ts)))
            date_str = dt.strftime("%Y-%m-%d")
            v = extract_val(item, "avg", "value")
            if v is not None: by_date[date_str]["wind"].append(v)
        except: continue

    # Construire la liste finale journalière
    daily = []
    for date_str in sorted(by_date.keys()):
        ...
        daily.append({
            ...
            "day":   dt.day,
            "avg":   mean(bd["avg"]),
            "hi":    round(max(bd["hi"]), 1) if bd["hi"] else None,
            "lo":    round(min(bd["lo"]), 1) if bd["lo"] else None,
            "hum":   mean(bd["hum"]),
            "rain":  round(sum(bd["rain"]), 1) if bd["rain"] else 0,
            "solar": mean(bd["solar"]),
            "pres":  mean(bd["pres"]),
            "wind":  mean(bd["wind"]),
        })
    ...
    for d in daily:
        m = d["month"]
        if d["avg"]  is not None: monthly[m]["t"].append(d["avg"])
        if d["hi"]   is not None: monthly[m]["hi"].append(d["hi"])
        if d["lo"]   is not None: monthly[m]["lo"].append(d["lo"])
        if d["rain"] is not None: monthly[m]["r"].append(d["rain"])
        if d["solar"]is not None: monthly[m]["s"].append(d["solar"])
        if d["hum"]  is not None: monthly[m]["h"].append(d["hum"])
        if d["pres"] is not None: monthly[m]["p"].append(d["pres"])
        if d["wind"] is not None: monthly[m]["w"].append(d["wind"])
    ...
    for m in range(1,13):
        ...
        agg[m] = {
            "name":     mn[m-1],
            "avg_t":    round(sum(mv["t"])/len(mv["t"]),1)  if mv["t"]  else None,
            "max_t":    round(max(mv["hi"]),1)               if mv["hi"] else None,
            "min_t":    round(min(mv["lo"]),1)               if mv["lo"] else None,
            "rain":     round(sum(mv["r"]),1)                if mv["r"]  else 0,
            "solar":    round(sum(mv["s"])/len(mv["s"]),1)  if mv["s"]  else None,
            "hum":      round(sum(mv["h"])/len(mv["h"]),1)  if mv["h"]  else None,
            "pressure": round(sum(mv["p"])/len(mv["p"]),1)  if mv["p"]  else None,
            "n":        len(mv["t"]),
        }

    # Humidex mensuel
    for m in range(1,13):
        t = agg[m]["avg_t"]; h = agg[m]["hum"]
        if t is not None and h is not None:
            e = (h/100)*6.105*math.exp(17.27*t/(237.7+t))
            agg[m]["humidex"] = round(t + 0.33*e - 4, 1)
        else:
            agg[m]["humidex"] = None

    # Jours remarquables
    gel=defaultdict(int); chaud=defaultdict(int); pluie=defaultdict(int)
    for d in daily:
        m=d["month"]
        if d["lo"]   is not None and d["lo"]   < 0:   gel[m]   += 1
        if d["hi"]   is not None and d["hi"]   >= 30:  chaud[m] += 1
        if d["rain"] is not None and d["rain"] > 1:    pluie[m] += 1

    # Heatmap [mois][jour 0-30]
    heatmap = []
    for m in range(1,13):
        row = []
    ...
    # KPI globaux
    all_hi   = [d["hi"]   for d in daily if d["hi"]   is not None]
    all_lo   = [d["lo"]   for d in daily if d["lo"]   is not None]
    all_rain = [d["rain"] for d in daily if d["rain"] is not None]

    result = {
        "daily":    daily,
        "monthly":  {str(k): v for k,v in agg.items()},
        "heatmap":  heatmap,
        "gel":      {str(k): v for k,v in gel.items()},
        "chaud":    {str(k): v for k,v in chaud.items()},
        "pluie":    {str(k): v for k,v in pluie.items()},
    ...
    DATA_FILE.write_text(json.dumps(result, ensure_ascii=False, indent=2))
    print(f"  → Max: {result['max_abs']}°C, Min: {result['min_abs']}°C, Pluie: {result['rain_total']}mm")
    return result
...
nav a{{font-size:13px;padding:6px 14px;border-radius:var(--radius);border:0.5px solid var(--border);
  background:var(--surface-muted);color:var(--text-secondary);text-decoration:none}}
nav a:hover{{color:var(--accent);border-color:rgba(42,120,214,.3)}}
...
<footer>Station météo personnelle · Colmar-Mittelharth · Alsace<br>
Données : <a href="https://www.ecowitt.net" style="color:var(--accent)">Ecowitt</a> · 
Site généré automatiquement</footer>
</div>
</body>
</html>"""
...
def build_dashboard(hist):
    """Page dashboard avec graphiques historiques (Chart.js)."""
    m   = hist["monthly"]
    mn  = ["Jan","Fév","Mar","Avr","Mai","Juin","Juil","Août","Sep","Oct","Nov","Déc"]

    # Construire les arrays JS depuis les données mensuelles
    def arr(key, sub=None):
        vals = []
        for i in range(1,13):
            mv = m.get(str(i), {})
            v = mv.get(key) if sub is None else (mv.get(key) or {}).get(sub)
            vals.append(v)
    ...
    gel_arr   = json.dumps([hist["gel"].get(str(i),0)   for i in range(1,13)])
    chaud_arr = json.dumps([hist["chaud"].get(str(i),0) for i in range(1,13)])
    pluie_arr = json.dumps([hist["pluie"].get(str(i),0) for i in range(1,13)])

    # Daily arrays
    daily = hist["daily"]
    d_dates = json.dumps([d["date"] for d in daily])
    d_avg   = json.dumps([d["avg"]  for d in daily])
    d_hi    = json.dumps([d["hi"]   for d in daily])
    d_lo    = json.dumps([d["lo"]   for d in daily])

    heatmap_js = json.dumps(hist["heatmap"])

    kpi_max  = hist.get("max_abs","—")
    kpi_min  = hist.get("min_abs","—")
    kpi_rain = hist.get("rain_total", 0)
...
:root{{
  ...
  --accent-bg:#e6f1fb;--accent-border:rgba(42,120,214,.3);
  --grid:#e1e0d9;
}}
@media(prefers-color-scheme:dark){{
  :root{{--bg:#111110;--surface:#1e1e1c;--surface-muted:#252523;
...
.hm-label-day{{font-size:10px;color:var(--text-muted);display:flex;align-items:center;justify-content:center}}
...
<header>
  <h1>📊 Dashboard historique</h1>
  <p>Station Colmar-Mittelharth · Généré le {gen}</p>
</header>
...
<div class="kpi-grid">
  <div class="kpi-card"><div class="kpi-label">Temp. max</div><div class="kpi-value">{kpi_max} °C</div></div>
  <div class="kpi-card"><div class="kpi-label">Temp. min</div><div class="kpi-value">{kpi_min} °C</div></div>
  <div class="kpi-card"><div class="kpi-label">Précipitations</div><div class="kpi-value">{kpi_rain} mm</div><div class="kpi-sub">12 derniers mois</div></div>
...
<div class="section">
  ...
  <div class="chart-note">Règle Walter-Lieth : échelle P = 2 × T · Période sèche : T au-dessus de P</div>
</div>
...
<div class="section">
  ...
  <div style="display:flex;align-items:center;gap:8px;margin-top:12px;font-size:12px;color:var(--text-muted)">
    <span>Froid</span>
    <div id="hmLegendBar" style="flex:1;height:10px;border-radius:4px"></div>
    <span>Chaud</span>
  </div>
</div>
...
<script>
const months={mn};
const avgTemp={avg_t}, maxTemp={max_t}, minTemp={min_t};
const rain={rain}, solar={solar}, humidity={hum}, pressure={pres}, humidex={humidex};
const joursGel={gel_arr}, joursChaud={chaud_arr}, joursPluie={pluie_arr};
const dailyDates={d_dates}, dailyAvg={d_avg}, dailyHi={d_hi}, dailyLo={d_lo};
const heatmap={heatmap_js};

const gc=()=>window.matchMedia('(prefers-color-scheme:dark)').matches?'#2c2c2a':'#e1e0d9';
const tc=()=>'#898781';

// KPI jours remarquables
document.getElementById('kpi-gel').textContent   = joursGel.reduce((a,b)=>a+b,0);
document.getElementById('kpi-chaud').textContent = joursChaud.reduce((a,b)=>a+b,0);
...
function showChart(mode){{
  ...
  const opts={{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},scales:{{x:{{ticks:{{color:tc(),autoSkip:false,maxRotation:0}},grid:{{color:gc()}}}},y:{{ticks:{{color:tc()}},grid:{{color:gc()}}}}}}}};
  const cfgs={{
    temp:{{type:'bar',data:{{labels:months,datasets:[{{label:'Max',data:maxTemp,backgroundColor:'rgba(216,90,48,.85)',borderRadius:4}},{{label:'Moy',data:avgTemp,backgroundColor:'rgba(42,120,214,.85)',borderRadius:4}},{{label:'Min',data:minTemp,backgroundColor:'rgba(27,175,122,.85)',borderRadius:4}}]}},options:opts}},
    rain:{{type:'bar',data:{{labels:months,datasets:[{{label:'Pluie',data:rain,backgroundColor:'rgba(42,120,214,.7)',borderRadius:4}}]}},options:opts}},
    solar:{{type:'line',data:{{labels:months,datasets:[{{label:'Solaire',data:solar,borderColor:'#eda100',backgroundColor:'rgba(237,161,0,.12)',borderWidth:2,pointBackgroundColor:'#eda100',fill:true,tension:0.4}}]}},options:opts}},
    hum:{{type:'line',data:{{labels:months,datasets:[{{label:'Hum',data:humidity,borderColor:'#1baf7a',backgroundColor:'rgba(27,175,122,.1)',borderWidth:2,pointBackgroundColor:'#1baf7a',fill:true,tension:0.4}}]}},options:{{...opts,scales:{{x:{{ticks:{{color:tc(),autoSkip:false,maxRotation:0}},grid:{{color:gc()}}}},y:{{min:0,max:100,ticks:{{color:tc()}},grid:{{color:gc()}}}}}}}}}},
    pressure:{{type:'line',data:{{labels:months,datasets:[{{label:'Pression',data:pressure,borderColor:'#4a3aa7',backgroundColor:'rgba(74,58,167,.1)',borderWidth:2,pointBackgroundColor:'#4a3aa7',fill:true,tension:0.4}}]}},options:opts}},
  }};
...
// Courbe journalière
const dlabels=dailyDates.map(d=>parseInt(d.slice(8))===1?months[parseInt(d.slice(5,7))-1]:'');
new Chart(document.getElementById('dailyChart'),{{
  type:'line',
  data:{{labels:dlabels,datasets:[
    {{label:'Max',data:dailyHi,borderColor:'rgba(216,90,48,.65)',backgroundColor:'rgba(216,90,48,.08)',borderWidth:1.5,pointRadius:0,fill:'+1',tension:0.2}},
    {{label:'Moy',data:dailyAvg,borderColor:'#2a78d6',backgroundColor:'rgba(42,120,214,.07)',borderWidth:2,pointRadius:0,fill:false,tension:0.2}},
    {{label:'Min',data:dailyLo,borderColor:'rgba(27,175,122,.65)',backgroundColor:'rgba(27,175,122,.08)',borderWidth:1.5,pointRadius:0,fill:'-1',tension:0.2}},
  ]}},
  options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}},tooltip:{{mode:'index',intersect:false,callbacks:{{title:c=>dailyDates[c[0].dataIndex],label:c=>c.dataset.label+' : '+c.parsed.y.toFixed(1)+' °C'}}}}}},scales:{{x:{{ticks:{{color:tc(),maxRotation:0,autoSkip:false,callback:(v,i)=>dlabels[i]}},grid:{{color:gc()}}}},y:{{ticks:{{color:tc(),callback:v=>v+' °C'}},grid:{{color:gc()}}}}}}}},
}});

// Walter-Lieth canvas pur
function drawWL(){{
  ...
  function xPx(i){{return ml+(i/(n-1))*cW;}}
  function yPxT(t){{return mt+(1-(t-Tmin)/(Tmax_ext-Tmin))*cH;}}
  function yPxP(p){{const te=p<=100?p/2:50+(p-100)/10; return yPxT(te);}}
  const y100=yPxT(50);
  ...
  // Zones
  const avgT_=avgTemp.filter(v=>v!==null);
  const rain_=rain.filter(v=>v!==null);
  for(let i=0;i<n-1;i++){{
  ...
  months.forEach((_,i)=>{{if(rain[i]==null)return;i===0||rain.slice(0,i).every(v=>v==null)?ctx.moveTo(xPx(i),yPxP(rain[i])):ctx.lineTo(xPx(i),yPxP(rain[i]));}});
  ctx.stroke();
  ...
  months.forEach((_,i)=>{{if(avgTemp[i]==null)return;i===0||avgTemp.slice(0,i).every(v=>v==null)?ctx.moveTo(xPx(i),yPxT(avgTemp[i])):ctx.lineTo(xPx(i),yPxT(avgTemp[i]));}});
  ctx.stroke();
  ...
  ctx.fillStyle='rgba(42,120,214,.8)';ctx.font='10px system-ui,sans-serif';ctx.textAlign='left';
  ctx.fillText('100 mm',ml+cW+4,y100+4);
  // Axes
...
for(let d=1;d<=31;d++){{const c=document.createElement('div');c.className='hm-label-day';c.style.fontSize='9px';c.textContent=d;grid.appendChild(c);}}
...
heatmap.forEach((mdata,mi)=>{{
  ...
  for(let d=0;d<31;d++){{
    ...
    if(val!==null&&val!==undefined&&d<monthDays[mi]){{
      ...
      cell.addEventListener('mousemove',e=>{{tooltip.style.display='block';tooltip.style.left=(e.clientX+12)+'px';tooltip.style.top=(e.clientY-28)+'px';tooltip.textContent=months[mi]+' '+(d+1)+' → '+val.toFixed(1)+' °C';}});
      cell.addEventListener('mouseleave',()=>tooltip.style.display='none');
    }}else cell.style.background='transparent';
    grid.appendChild(cell);
  }}
}});
document.getElementById('hmLegendBar').style.background='linear-gradient(to right,rgb(30,60,180),rgb(80,140,220),rgb(150,210,200),rgb(250,220,100),rgb(230,100,30),rgb(180,20,20))';
...
months.forEach((m,i)=>{{
  ...
  tr.innerHTML=`<td>${{m}}</td><td>${{b(joursGel[i],'badge-gel')}}</td><td>${{b(joursChaud[i],'badge-chaud')}}</td><td>${{b(joursPluie[i],'badge-pluie')}}</td>`;
  tbody.appendChild(tr);
}});
const sum=arr=>arr.filter(v=>v!==null).reduce((a,b)=>a+b,0);
document.getElementById('t-gel').innerHTML   =`<span class="badge badge-gel">${{sum(joursGel)}}</span>`;
document.getElementById('t-chaud').innerHTML =`<span class="badge badge-chaud">${{sum(joursChaud)}}</span>`;
document.getElementById('t-pluie').innerHTML =`<span class="badge badge-pluie">${{sum(joursPluie)}}</span>`;

// Graphiques secondaires
new Chart(document.getElementById('rainChart'),{{type:'bar',data:{{labels:months,datasets:[{{label:'Pluie',data:rain,backgroundColor:'rgba(42,120,214,.7)',borderRadius:4}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},scales:{{x:{{ticks:{{color:tc(),autoSkip:false,maxRotation:0}},grid:{{color:gc()}}}},y:{{ticks:{{color:tc(),callback:v=>v+' mm'}},grid:{{color:gc()}}}}}}}}}});
new Chart(document.getElementById('solarChart'),{{type:'line',data:{{labels:months,datasets:[{{label:'Solaire',data:solar,borderColor:'#eda100',backgroundColor:'rgba(237,161,0,.12)',borderWidth:2,pointBackgroundColor:'#eda100',fill:true,tension:0.4}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},scales:{{x:{{ticks:{{color:tc(),autoSkip:false,maxRotation:0}},grid:{{color:gc()}}}},y:{{ticks:{{color:tc(),callback:v=>v+' W/m²'}},grid:{{color:gc()}}}}}}}}}});
new Chart(document.getElementById('humChart'),{{type:'line',data:{{labels:months,datasets:[{{label:'Hum',data:humidity,borderColor:'#1baf7a',backgroundColor:'rgba(27,175,122,.1)',borderWidth:2,pointBackgroundColor:'#1baf7a',fill:true,tension:0.4}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},scales:{{x:{{ticks:{{color:tc(),autoSkip:false,maxRotation:0}},grid:{{color:gc()}}}},y:{{min:0,max:100,ticks:{{color:tc(),callback:v=>v+' %'}},grid:{{color:gc()}}}}}}}}}});
</script>
</body>
</html>"""
...
def build_climate(hist):
    """Page climatologie simple."""
    m = hist["monthly"]
    mn = ["Jan","Fév","Mar","Avr","Mai","Juin","Juil","Août","Sep","Oct","Nov","Déc"]
    rows = ""
    for i in range(1,13):
        ...
        def fmt(k, u="", dec=1):
            v = mv.get(k)
            return f"{v:.{dec}f}{u}" if v is not None else "—"
        rows += f"""<tr>
          <td>{mn[i-1]}</td>
          <td>{fmt('avg_t',' °C')}</td>
          <td style="color:#d85a30">{fmt('max_t',' °C')}</td>
          <td style="color:#2a78d6">{fmt('min_t',' °C')}</td>
          <td>{fmt('rain',' mm')}</td>
          <td>{fmt('solar',' W/m²',0)}</td>
          <td>{fmt('hum',' %',0)}</td>
          <td>{fmt('pressure',' hPa',0)}</td>
        </tr>"""
...
<header>
  <h1>🌍 Climatologie</h1>
  <p>Station Colmar-Mittelharth · Statistiques mensuelles</p>
</header>
<nav>
  <a href="index.html">⚡ En direct</a>
  <a href="dashboard.html">📊 Historique</a>
...
<div class="section">
  <div class="section-title">Moyennes mensuelles — 12 derniers mois</div>
  <table>
    <thead><tr>
...
if __name__ == "__main__":
    ...
    # Toujours récupérer le temps réel
    live = fetch_realtime()
    build_index(live)

    # Historique uniquement si demandé (ou si le fichier n'existe pas)
    import sys
    if "--full" in sys.argv or not DATA_FILE.exists():
        hist = fetch_history()
    else:
        hist = json.loads(DATA_FILE.read_text())

    build_dashboard(hist)
    build_climate(hist)
